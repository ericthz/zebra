...
**Zebra 是 AI Agent 的全功能最小实现，帮助掌握 AI Agent 原理，把各个技能、对话与推理、记忆、知识、工具、多 Agent、评测、安全、可观测、规模化，用最小可读的代码实现出来，让你看懂每个技能背后的原理。**
...
```
┌──────────────────────┐        ┌─────────────────────────────────┐        ┌─────────────────────────────────┐        ┌────────────────────────────┐
│ cmd/                 │        │ internal/server —— HTTP API/SSE │        │ agent · tool · memory · mcp     │        │ provider · safety · rag/kg │
│ server · zebra · mcp │  ───►  │ 会话·鉴权·限流·WebUI·评测          │  ───►  │ 编排/规划·工具+沙箱·分层记忆        │  ───►  │ 多协议LLM/语音·注入/审核      │
│                      │        │ 影子·热更新·异步任务               │        │ 画像/遗忘·MCP协议栈(HTTP/stdio)    │        │ 检索/重排·知识图谱           │
└──────────────────────┘        └─────────────────────────────────┘        └─────────────────────────────────┘        └────────────────────────────┘
```
